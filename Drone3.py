...
class Crazyflie(Supervisor):
    def __init__(self, cfg):
        ...
        # Defining the square and triangle waypoints for Drone 3
        self.square_positions = [
            [1.0, 1.0, 1.4],  # Top right (Initial)
            [0.0, 1.0, 1.4],  # Top left
            [0.0, 0.0, 1.4],  # Bottom left
            [1.0, 0.0, 1.4]   # Bottom right
        ]
        
        self.triangle_positions = [
            [0.5, 1.0, 1.4],  # Top middle
            [1.0, 1.0, 1.4],  # Top right
            [0.0, 0.0, 1.4],  # Bottom left
            [0.5, 0.5, 1.4]   # Center
        ]

        # Assigning a fixed height to avoid collisions
        self.flight_height = 1.4  # Drone 3's fixed flight height
    ...
    def setWaypointPos(self, waypoint=None):
        ...
        if current_time - self.last_time >= self.hover_time:
            if waypoint is None:
                ...
                print(f"Drone 3 has reached waypoint: {self.waypointpos}")
                ...
                print(f"Drone 3 set to custom waypoint: {self.waypointpos}")
...
if __name__ == '__main__':
    ...
    cf.setWaypointPos(waypoint=[1.5932573, -1.3207, 2.00949])     
    while True:
