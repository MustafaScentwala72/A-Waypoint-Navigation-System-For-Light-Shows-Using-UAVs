...
class Crazyflie(Supervisor):
    def __init__(self, cfg):
        ...
        # Defining the square and triangle waypoints for Drone 2
        self.square_positions = [
            [1.0, 0.0, 1.2],  # Bottom right (Initial)
            [1.0, 1.0, 1.2],  # Top right
            [0.0, 1.0, 1.2],  # Top left
            [0.0, 0.0, 1.2]   # Bottom left
        ]
        
        self.triangle_positions = [
            [1.0, 0.0, 1.2],  # Bottom right
            [0.5, 1.0, 1.2],  # Top middle
            [0.0, 0.0, 1.2],  # Bottom left
            [0.5, 0.5, 1.2]   # Center
        ]

        # Assigning a fixed height to avoid collisions
        self.flight_height = 1.2  # Drone 2's fixed flight height
    ...
    def setWaypointPos(self, waypoint=None):
        ...
        if current_time - self.last_time >= self.hover_time:
            if waypoint is None:
                ...
                print(f"Drone 2 has reached waypoint: {self.waypointpos}")
                ...
                print(f"Drone 2 set to custom waypoint: {self.waypointpos}")
...
if __name__ == '__main__':
    ...
    cf.setWaypointPos(waypoint=[1.5932573, -1.3207, 1.50949])     
    while True:
