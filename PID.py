...
class PID(object):
    ...
    def __init__(
        ...
        """
        Initializing a new PID controller.
        ...
        try:
            # Getting monotonic time to ensure that time deltas are always positive
    ...
    def __call__(self, reference_, vel=None, dt=None):
        ...
        if self.sample_time is not None and dt < self.sample_time and self._last_output is not None:
            # Only updating every sample_time seconds
            return self._last_output
        
        # Computing feedforward term
        ...
        # Checking if must map the error
        if self.error_map is not None:
            error = self.error_map(error)

        # Computing the proportional term
        if not self.proportional_on_measurement:
            # Regular proportional-on-error, simply set the proportional term
            self._proportional = self.Kp * error
        else:
            # Adding the proportional error on measurement to error_sum
            self._proportional -= self.Kp * d_input

        # Computing integral and derivative terms
