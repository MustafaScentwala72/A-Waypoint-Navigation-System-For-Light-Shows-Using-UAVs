...
class Crazyflie(Supervisor):
    def __init__(self, cfg):
        ...
        # Defining the square and triangle waypoints for Drone 1
        self.square_positions = [
            [0.0, 0.0, 1.0],  # Bottom left (Initial)
        ...
        self.triangle_positions = [
            ...
            [1.0, 0.0, 1.0],  # Bottom right
            [0.5, 1.0, 1.0],  # Top middle
            [0.5, 0.5, 1.0]   # Center
        ]

        # Assigning a fixed height to avoid collisions
        self.flight_height = 1.0  # Drone 1's fixed flight height
    ...
    def setWaypointPos(self, waypoint=None):
        ...
        if current_time - self.last_time >= self.hover_time:
            if waypoint is None:
                ...
                print(f"Drone 1 has reached waypoint: {self.waypointpos}")
                ...
                print(f"Drone 1 set to custom waypoint: {self.waypointpos}")
...
if __name__ == '__main__':
    ...
    cf.setWaypointPos(waypoint=[1.5932573, -1.3207, 1.00949])     
    while True:
